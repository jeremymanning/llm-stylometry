...
It is possible to fine-tune large languages to write in the "style' or "voice" of a given author (see e.g., \cite{10.1093/llc/fqaf035}). Generally, the mathematical encoding -- or ``measure" -- of a writer's style falls under the heading of {\em stylometry}. Stylometry generally elides the complex collection of factors that may go
into an author's ``voice'', focusing instead on word-usage statistics derived from the
text~\citep{NealEtal17}.  Many mark
...
conclusions~\citep{Luto97}. Stylometric methods have been used at scale to understand the evolution of style in literature~\citep{HughEtal12} as well as judicial writing\citep{Carlson2016}, while also contributing to debates around uncertain authorship~\citep{MostWall63,MostWall84,NiloBino03,Juol08}. 

In this paper we introduce a new stylometric approach, {\em Predictive comparison testing} (PCT), based on LLMs. PCT derives from the hypothesis that given two LLMs of a fixed architecture and fixed broad  training, separately fine-tuned models ($A$ Aand $B)$  on similarly sized corpora of Author $A$ and $B$, that a held out work by Author $A$ will exhibit small predictive loss when tested with model $A$ than with model $B$. Comparison of prediction at the level of sentences and using textual information is put forward in \citep{rezaei-2025-detecting}. Our work differs both in the scale and reliance purely on the loss function. 

Our results, using a small data set of publicly available work from Project Gutenberg (overlapping that used in \cite{rezaei-2025-detecting}) bear out this hypothesis. From this we build an author style metric. We believe that these results suggest that this technique holds promise as a new technique for machine reading approaches to text-based disciplines ~\citep{More17,More00,Holmes} and the practices of cultural analytics~\citep{Underwood_2013}.  PCT also suggests a natural approach to author attribution in the case in which there is a finite set of possiblities: assign the work to the author whose PCT produces the least loss. To this we consider a well-known example of questioned attribution of the 15th book in the {\emph{Oz}} series and scholarly opinion~\citep{NiloBino03}, further validating our approach. 

%In this paper we exploit this technique to introduce {\em predictive comparison testing}, an a newLLM-based stylometric measure. 
...
%practice of ``distant reading"~\citep{Mo17, More00} re(the machine reading of
...
We begin by applying this approach to a set of eight selected authors, and then narrow our focus to compare the two \emph{Oz} series authors, Baum and Thompson, in greater detail. 

\subsection{Data and Data Preprocessing}



We first consider a set of eight authors: Jane Austen, L. Frank Baum, Charles Dickens, F.
...
Appendix~\ref{sec:appendixA}. We then take on an author attribution question and (re-)visit scholarly debate around the 15th book in the \emph{Oz} series, and using PCT confirm attribution to Rosemary Plumly Thompson, as opposed to the series originator, L. Frank Baum.  
...
For each author, we compute the predictive loss of the corresponding
model on a held-out book by that author, as well as on one randomly selected
book from each of the other authors. We then compute the average next-token cross-entropy with sliding windows.
...
Once again, after each training epoch, we compute the loss of every model on its
corresponding held-out book as well as on one randomly selected book from the
other author's corpora. For the specific case of Baum and Thompson (and following \citep{NiloBino03}, we include
...
\begin{figure*}[thb]
  ...
  \caption{\textbf{A.} Average cross-entropy loss on evaluation texts for models trained on Baum and Thompson, each trained with a different random seed; 
  %we report   mean cross-entropy loss across 10 random seeds, 
...
\textbf{B.}  Average cross-entropy loss on evaluation texts across all eight authors. Error bars denote 95\% confidence intervals over 10 random seeds.
  \textbf{C.} $t$-test results by author for first 500 training epochs.
  \textbf{D.} Averaged $t$-test results for first 500 training epochs.}
...
The  paired $t$-test also provides further validation for the {\emph{Oz} authorship question. On the average cross-entropy losses of the two models
...
$t(9) = 20.723, p = 6.6 \times 10^{-9}$. This provides strong evidence
that the Thompson-trained models predict the contested text more accurately,
aligning with prevailing literary consensus regarding its authorship.
...
styles:
%-- if the loss of predicting tokens in a work of author $j$ from a model
...
%similar or nearby. 
Let $L_i(j)$ denote the average loss of a work of author $j$
...
The main limitations of this paper are (1) the lack of breadth of experiments as well as
(2) the oft-acknowledged opacity of the LLM. 
%The results in this paper serve as a
...
%results on a classic stylometric test are promising.
Further testing is
needed to understand what kinds of writing features are being picked up by the
LLM. Finally, deploying this idea at scale would require fine-tuning one model for every writer of interest, a task that would require significant computational and textual resources. 
...
\begin{center}
{\Large \bfseries Appendix A: Authors, Books, Tokens \par}
\vspace{1em}
\begin{tabular}{@{}ll|ll@{}}
\toprule
